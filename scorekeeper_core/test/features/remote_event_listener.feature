Feature: Receiving domain events from remote instances
  The Scorekeeper instance can be wired to receive domain events from remote instances.
  These remote domain events are to be validated, stored and applied appropriately.
  These scenario's will cover the basic functionality, the more complex conflict scenario's are described in the event_synchronization feature.
